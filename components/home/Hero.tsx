  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < 130) {
            ...
            ctx.strokeStyle = `rgba(245,180,0,${0.18 * (1 - dist / 130)})`;
            ctx.lineWidth = 0.8;
            ctx.stroke();
          }
        }
      ...
      nodes.forEach((n) => {
        ...
        ctx.fillStyle = "rgba(245,180,0,0.45)";
        ctx.fill();
      });
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-[55%] h-[70%] rounded-full opacity-30"
          style={{ background: "radial-gradient(ellipse at 80% 20%, #FFF3B0 0%, transparent 65%)" }} />
        <div className="absolute bottom-0 left-0 w-[40%] h-[50%] rounded-full opacity-20"
          style={{ background: "radial-gradient(ellipse at 20% 80%, #FFFACD 0%, transparent 60%)" }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 lg:py-32 w-full">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <span className="relative inline-block">
              ...
              <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 300 8" fill="none">
                <path d="M2 6C60 2 180 2 298 6" stroke="#F5B400" strokeWidth="3" strokeLinecap="round" />
              </svg>
            </span>
          ...
          >
            <Link
              href="/courses"
              className="btn-gold px-8 py-4 rounded-full text-base gap-2 shadow-gold"
            >
              Explore Courses <ArrowRight className="w-5 h-5" />
            </Link>
            <Link
              href="/services"
              className="btn-outline px-8 py-4 rounded-full text-base gap-2"
            >
              Our Services
            </Link>
            <a
          ...
          >
            {[
              ...
              <div key={s.label} className="bg-white flex flex-col items-center justify-center py-6 px-4">
